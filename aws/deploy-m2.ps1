# Flight Price Notifier - M2 (ECPay paywall) backend deploy.
# Run from anywhere:
#   powershell -ExecutionPolicy Bypass -File C:\Users\User\fare-finder-m1\aws\deploy-m2.ps1
# Safe to re-run: existing functions are updated, missing ones are created.
...
$work        = Join-Path $env:TEMP "flight-m2-deploy"
$statusQueue = "https://sqs.us-east-1.amazonaws.com/767397924129/flight-status-queue"
$fareQueue   = "https://sqs.us-east-1.amazonaws.com/767397924129/flight-fare-queue"
$bucket      = "flight-config-767397924129"
...
Write-Host "1/2 IAM: add flight-status-queue to the flight-data policy" -ForegroundColor Cyan
...
Start-Sleep -Seconds 10   # let the new IAM permissions propagate before the functions use them

Write-Host "2/2 Lambdas" -ForegroundColor Cyan
# existing functions (M1) - new code, same env + SITE_URL
...
Deploy-Fn "flight-list-subscriptions" "flight-list-subscriptions" 10 @{ SUPABASE_URL = $sbUrl; SUPABASE_PUBLISHABLE_KEY = $sbKey }
Deploy-Fn "flight-parser"             "flight-parser"             30 @{ FARE_QUEUE_URL = $fareQueue; CONFIG_BUCKET = $bucket }
# new M2 functions (the two callbacks share one source file)
...
Deploy-Fn "flight-ecpay-result"        "flight-ecpay-result"        5  @{ SITE_URL = $site }
...
Write-Host "DONE - tell Claude so it can wire the API routes and the status queue." -ForegroundColor Cyan
